        ...
        <div className={`px-4 w-full flex justify-center min-h-screen bg-${bgMode} bg-no-repeat bg-center bg-cover`}>
            <div className="lg:max-w-2xl w-full mx-auto text-center pt-10">
                <div className="bg-slate-400/60 p-5 rounded-lg mx-auto">
                    ...
                    <form action="#" className="mt-24 flex items-center" onSubmit={handleGetWeather}>
                        <input type="search" placeholder={data.name || 'نام شهر ...'} className="w-full bg-transparent text-orange-700 text-sm sm:text-base p-3 outline-none border-slate-600 border-b-[3px] duration-200 font-bold focus:border-b-orange-600 placeholder:font-semibold placeholder:text-slate-700" value={query} onChange={(e) => setQuery(e.target.value)} />
                        <button type="submit" className="bg-orange-600 text-white text-sm mt-4 mr-2 px-4 py-3 duration-100 rounded-md hover:bg-orange-700 focus:ring-4 ring-orange-500">
...

                {
                    (loading) ? (
                        ...
                        <div className="bg-slate-500/90 animate-show rounded-md py-5 px-3 mt-16 mx-auto duration-500 hover:-translate-y-1">
                            ...
                            <h3 className="font-bold text-base md:text-lg text-gray-400 mt-5">{ data.weather[0].main }</h3>
                        </div>
                    ) : (
                        <div className="bg-slate-500/90 rounded-lg py-5 px-3 mt-16 mx-auto">
